...
const STRINGS = {
  ...
  'info.buildOpts': ['With Formation set to Auto, these are three real squads for the same budget - one built for a defensive, one for a balanced, and one for an attacking shape - each scored on its own true forecast (best XI + captain) over the selected horizon, so the label is a genuine formation-vs-xP tradeoff. Lock a specific formation first and these become three jittered takes on that one shape instead. Tap to switch, fine-tune on the pitch.', 'כשהפורמציה על אוטומטי, אלה שלושה הרכבים אמיתיים לאותו תקציב - אחד בנוי להגנתי, אחד למאוזן ואחד להתקפי - כל אחד עם הצפי האמיתי שלו (11 הטובים + קפטן) לאופק שנבחר, כך שהתווית היא טרייד-אוף אמיתי בין פורמציה ל-xP. נועלים פורמציה מסוימת מראש ואז אלה הופכים לשלוש גרסאות של אותה פורמציה. מקישים כדי להחליף, מלטשים על המגרש.'],
